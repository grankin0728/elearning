package com.suusoft.elistening.widgets.recyclerview;
...
public class EndlessRecyclerOnScrollListener extends RecyclerView.OnScrollListener {
    ...
    public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
        ...
        if (!isEnded) {
            if (mLayoutType == LayoutManagerType.STAGGERED_LAYOUT) {
                // chua test STAGGERED_LAYOUT
                if (!isLoading && recyclerView.getLayoutManager().getItemCount() <= (((StaggeredGridLayoutManager) recyclerView.getLayoutManager()).findLastVisibleItemPositions(null)[0])+ 3) {
                    onLoadMore();
                }
            } else {
                if (!isLoading && recyclerView.getLayoutManager().getItemCount() <= (((LinearLayoutManager) recyclerView.getLayoutManager()).findLastVisibleItemPosition() + 3)) {
                    onLoadMore();
                }
            }
        }
